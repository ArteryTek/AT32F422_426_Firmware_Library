...
/** @addtogroup 422_I2C_communication_smbus I2C_communication_smbus
  * @{
  */
...
uint8_t tx_buf[BUF_SIZE+1] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00};
uint8_t rx_buf[BUF_SIZE+1] = {0};
...
void i2c_lowlevel_init(i2c_handle_type* hi2c)
{
  ...
  if(hi2c->i2cx == I2Cx_PORT)
  {
    ...
    i2c_smbus_enable(hi2c->i2cx, TRUE);
  }
}
...
int main(void)
{
  ...
  while(1)
  {
    ...
    /* compare the data and pec value */
    if((buffer_compare(tx_buf, rx_buf, BUF_SIZE) == 0) && (rx_buf[BUF_SIZE] == hi2cx.pec_val))
    {
      at32_led_on(LED3);
    }
    else
