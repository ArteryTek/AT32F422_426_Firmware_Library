...
/** @addtogroup 422_ADC_voltage_monitoring ADC_voltage_monitoring
  * @{
  */
...
static void adc_config(void)
{
  ...
  adc_voltage_monitor_threshold_value_set(ADC1, 0x555, 0x000);
...
int main(void)
{
  ...
  while(1)
  {
    ...
    delay_sec(1);
    if(error_times_index != (adc1_overflow_flag + vmor_flag_index + adc1_conversion_fail_flag))
    {
      /* printf flag when error occur */
      error_times_index = adc1_overflow_flag + vmor_flag_index + adc1_conversion_fail_flag;
      at32_led_on(LED3);
      at32_led_on(LED4);
      printf("error occur\r\n");
