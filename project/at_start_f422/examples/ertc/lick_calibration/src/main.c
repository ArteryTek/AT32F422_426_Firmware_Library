...
/** @addtogroup AT32F422_periph_examples
  * @{
  */

/** @addtogroup 422_ERTC_lick_calibration ERTC_lick_calibration
  * @{
  */
...
int main(void)
{
  ...
  /* get the lick frequency: TMR4 is used to measure the lick frequency */
...
/**
  * @brief  configures TMR4 to measure the lick oscillator frequency.
  * @param  none
...
uint32_t lick_frequency_get(void)
{
  ...
  /* enable TMR4 apb1 clocks */
  crm_periph_clock_enable(CRM_TMR4_PERIPH_CLOCK, TRUE);

  /* connect internally the TMR4_ch1 to the lick clock output */
  tmr_iremap_config(TMR4, TMR4_ERTCCLK);

  /* configure TMR4 divider */
  tmr_div_value_set(TMR4, 0);
  tmr_event_sw_trigger(TMR4, TMR_OVERFLOW_SWTRIG);

  /* TMR4 channel4 input capture mode configuration */
  ...
  tmr_input_channel_divider_set(TMR4, TMR_SELECT_CHANNEL_1, TMR_CHANNEL_INPUT_DIV_8);

  /* enable TMR4 interrupt channel */
  ...
  /* wait the TMR4 measuring operation to be completed */
  while(capture_number != 2);

  /* deinitialize the TMR4 peripheral */
  tmr_reset(TMR4);
