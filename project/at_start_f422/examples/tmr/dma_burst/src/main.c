...
/** @addtogroup 422_TMR_dma_burst TMR_dma_burst
  * @{
  */
...
int main(void)
{
  ...
  /* tmr1 configuration: generate 1 pwm signal using the dma burst mode:
     ...
     system_core_clock is set to 180 mhz .
     the tmr1 period is 2.44 khz: tmr1 frequency = tmr1 counter clock/(pr + 1)
                                               = 10 mhz / 4096 = 2.44khz khz
     tmr1 channel1 duty cycle = (tim1_c1dt/ tmr1_pr)* 100 = 33.33% */

  tmr_base_init(TMR1, 0xFFFF, (crm_clocks_freq_struct.sclk_freq/10000000)-1);
