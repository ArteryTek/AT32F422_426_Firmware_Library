...
/**
  ...
  * @param  usd_ssb0: the system setting byte0
...
/**
  * @brief  program the flash system setting byte in usd: boot0_sw / boot0 / nrst_mode
  * @param  usd_ssb1: the system setting byte1
  * @note   this parameter usd_ssb1 must contain a combination of all the following 3 types of data
  *         type 1: boot0_sw, configure the boot0_sw.
  *         this data can be one of the following values:
  *         - USD_BOOT0_SW_DISABLE: boot0 state refer to pin.
  *         - USD_BOOT0_SW_ENABLE: boot0 state refer to bit.
  *         type 2: boot0, configure the boot0.
  *         this data can be one of the following values:
  *         - USD_BOOT0_LOW: nboot0 is 0
  *         - USD_BOOT0_HIGH: nboot0 is 1
  *         type 3: nrst_mode, select nrst mode.
  *         this data can be one of the following values:
  *         - USD_NRST_MODE_NRST: nrst pin with nrst mode
  *         - USD_NRST_MODE_GPIO: nrst pin with gpio mode
  * @retval status: the returned value can be: FLASH_PROGRAM_ERROR,
  *         FLASH_EPP_ERROR, FLASH_OPERATE_DONE or FLASH_OPERATE_TIMEOUT.
  */
...
/**
  * @brief  return the flash system setting byte0 status.
  * @param  none
...
/**
  * @brief  return the flash system setting byte1 status.
  * @param  none
  * @retval values from flash_usd register: boot0_sw(bit0), boot0(bit1),
  *         nrst_mode(bit2, biit3).
  */
uint8_t flash_ssb1_status_get(void)
{
  /* return the system setting byte status */
  return (uint8_t)(FLASH->usd >> 10);
}
