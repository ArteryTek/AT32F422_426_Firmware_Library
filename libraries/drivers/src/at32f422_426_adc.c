...
/**
  ...
  *         this parameter can be one of the following values:
  *         adc1
  *         - ADC_ORDINARY_TRIG_TMR1CH1     - ADC_ORDINARY_TRIG_TMR1CH2   - ADC_ORDINARY_TRIG_TMR1CH3  - ADC_ORDINARY_TRIG_TMR3CH3
  *         - ADC_ORDINARY_TRIG_TMR3TRGOUT  - ADC_ORDINARY_TRIG_TMR15CH1  - ADC_ORDINARY_TRIG_EXINT11  - ADC_ORDINARY_TRIG_SOFTWARE
...
/**
  ...
  *         this parameter can be one of the following values:
  *         adc1
  *         - ADC_PREEMPT_TRIG_TMR1TRGOUT  - ADC_PREEMPT_TRIG_TMR1CH4      - ADC_PREEMPT_TRIG_TMR4CH4  - ADC_PREEMPT_TRIG_TMR15CH2
  *         - ADC_PREEMPT_TRIG_TMR3CH4     - ADC_PREEMPT_TRIG_TMR15TRGOUT  - ADC_PREEMPT_TRIG_EXINT15  - ADC_PREEMPT_TRIG_SOFTWARE
...
flag_status adc_flag_get(adc_type *adc_x, uint16_t adc_flag)
{
  ...
  if((adc_flag == ADC_OTCF_FLAG) || (adc_flag == ADC_PTCF_FLAG))
  {
    if((adc_x->misc2 & ((adc_flag & 0x0300) >> 8)) == RESET)
    {
      status = RESET;
    }
    else
  ...
  {
    if((adc_x->sts & (adc_flag & 0x00FF)) == RESET)
    {
      status = RESET;
    }
    else
...
{
  adc_x->misc2 = ~((adc_flag & 0x0300) >> 8);

  adc_x->sts = ~(adc_flag & 0x00FF);
}
...
/**
  ...
  *         this parameter can be one of the following values:
  *         - ADC1.
  * @param  new_state: new state of trigger conversion fail auto conversion abort.
  *         this parameter can be: TRUE or FALSE.
  *         - note: if you want to use "conversion fail auto conversion abort" function(tcfaca = 1), you must
  *                 enable enable tcf interrupt first(tcfien = 1),and disable trigger conversion fail auto
  *                 processing mode(tcfapen = 0).
  * @retval none
  */
void adc_convert_fail_auto_abort_enable(adc_type *adc_x, confirm_state new_state)
{
  ...
  if(new_state == TRUE)
  {
    adc_x->misc1_bit.tcfapen = FALSE;
  }
} 

/**
  * @brief  set the xeoc sync mode of the specified adc peripheral.
  * @param  adc_x: select the adc peripheral.
  *         this parameter can be one of the following values:
  *         - ADC1.
  * @param  xeoc_sync_mode: select the adc xeoc sync mode.
  *         this parameter can be one of the following values:
  *         - ADC_XEOC_SYNC_RISING_EDGE
  *         - ADC_XEOC_SYNC_HIGH_LEVEL
  * @retval none
  */
void adc_xeoc_sync_mode_set(adc_type *adc_x, adc_xeoc_sync_type xeoc_sync_mode)
{
  adc_x->misc1_bit.xeoc_sm = xeoc_sync_mode;
}

/**
  * @brief  enable or disable adc trigger conversion fail auto processing mode.
  * @param  adc_x: select the adc peripheral.
  *         this parameter can be one of the following values:
  *         ADC1.
  * @param  new_state: new state of trigger conversion fail auto processing mode.
  *         this parameter can be: TRUE or FALSE.
  *         - note: when enale "trigger conversion fail auto processing mode" function(tcfapen = 1), the
  *                 conversion fail auto conversion abort function will be auto lose efficacy(tcfaca = 0)
  * @retval none
  */
void adc_fail_auto_processing_enale(adc_type *adc_x, confirm_state new_state)
{
  adc_x->misc1_bit.tcfapen = new_state;
  if(new_state == TRUE)
  {
    adc_x->ctrl1_bit.tcfaca = FALSE;
  }
}

/**
  * @brief  set the fail auto processing mode of the specified adc peripheral.
  * @param  adc_x: select the adc peripheral.
  *         this parameter can be one of the following values:
  *         - ADC1.
  * @param  fail_auto_processing_mode: select the fail auto processing mode.
  *         this parameter can be one of the following values:
  *         - ADC_AUTO_STORAGE_TEMPORARY_DATA
  *         - ADC_AUTO_CONVERSION_FAIL_CHANNEL
  *         - note: tcfaps only effective when enale "trigger conversion fail auto processing mode" function(tcfapen = 1)
  * @retval none
  */
void adc_fail_auto_processing_mode_set(adc_type *adc_x, adc_fail_auto_processing_mode_type fail_auto_processing_mode)
{
  adc_x->misc1_bit.tcfaps = fail_auto_processing_mode;
}

/**
  * @brief  get number of the specified adc peripheral conversion fail sequence.
  * @param  adc_x: select the adc peripheral.
  *         this parameter can be one of the following values:
  *         ADC1.
  * @param  adc_group_number: select the adc group number.
  *         this parameter can be one of the following values:
  *         - ADC_OSNF_NUMBER
  *         - ADC_PSNF_NUMBER
  * @retval the conversion fail sequence number for select group.
  */
uint16_t adc_conversion_fail_sequence_number_get(adc_type *adc_x, uint32_t adc_group_number)
{
  if(adc_group_number == ADC_OSNF_NUMBER)
  {
    return (uint16_t)(adc_x->misc2_bit.osnf);
  }
  else
  {
    return (uint16_t)(adc_x->misc2_bit.psnf);
  }
}

/**
  * @brief  clear number of the specified adc peripheral conversion fail sequence.
  * @param  adc_x: select the adc peripheral.
  *         this parameter can be one of the following values:
  *         ADC1.
  * @param  adc_group_number: select the adc group number.
