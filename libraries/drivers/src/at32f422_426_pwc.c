...
void pwc_flag_clear(uint32_t pwc_flag)
{
  ...
  if(pwc_flag & (PWC_WAKEUP_FLAG | PWC_STANDBY_FLAG))
  {
    flag_index = (pwc_flag & (PWC_WAKEUP_FLAG | PWC_STANDBY_FLAG)) << 2;
    pwc_flag = (pwc_flag & ((uint32_t)~(PWC_WAKEUP_FLAG | PWC_STANDBY_FLAG))) | flag_index;
  }
  PWC->ctrl |= pwc_flag;
} 
