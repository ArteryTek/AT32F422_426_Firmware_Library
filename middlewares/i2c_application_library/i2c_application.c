...
/** @addtogroup AT32F422_426_middlewares_i2c_application_library
  * @{
  */
...
void i2c_config(i2c_handle_type* hi2c)
{
  ...
  /* i2c digital filtering time = dfilters/apb1clk */
  i2c_digital_filter_value_set(hi2c->i2cx, 0x02);
...
/**
  * @brief  the smbus master transmits data through polling mode.
  * @param  hi2c: the handle points to the operation information.
  * @param  address: slave address.
  * @param  pdata: data buffer.
  * @param  size: data size.
...
i2c_status_type i2c_smbus_master_transmit(i2c_handle_type* hi2c, uint16_t address, uint8_t *pdata, uint16_t size, uint32_t timeout)
{
  ...
  i2c_smbus_mode_set(hi2c->i2cx, I2C_SMBUS_MODE_HOST);
...
/**
  * @brief  the smbus slave receive data through polling mode.
  * @param  hi2c: the handle points to the operation information.
  * @param  pdata: data buffer.
  * @param  size: data size.
  * @param  timeout: maximum waiting time.
...
i2c_status_type i2c_smbus_slave_receive(i2c_handle_type* hi2c, uint8_t *pdata, uint16_t size, uint32_t timeout)
{
  ...
  i2c_smbus_mode_set(hi2c->i2cx, I2C_SMBUS_MODE_DEVICE);
  ...
  while(size > 0)
  {
    ...
    if(size == 1)
    {
      /* enable pec transmit request */
      i2c_pec_transmit_enable(hi2c->i2cx, TRUE);
      hi2c->pec_val = i2c_pec_value_get(hi2c->i2cx);
    }
...
/**
  * @brief  the smbus master receive data through polling mode.
  * @param  hi2c: the handle points to the operation information.
  * @param  address: slave address.
  * @param  pdata: data buffer.
  * @param  size: data size.
...
/**
  * @brief  the smbus slave transmits data through polling mode.
  * @param  hi2c: the handle points to the operation information.
  * @param  pdata: data buffer.
  * @param  size: data size.
  * @param  timeout: maximum waiting time.
